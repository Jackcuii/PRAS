...
#sysprompt = "You are a strict prosecutor. You are going to sentence a company for they have flaws in their applications.  Figure out if there is any evidence that they behave poorly."
...
sysprompt = "You are a staff of a company. Here are some parts of an Privacy Policy and Data Safety Declaration of an application of your company. You are trying to verify that the Data Safety Declaration complies with the Privacy Policy below."
...
# url = "https://api.baizhi.ai/v1/chat/completions"
    ...
    ctnt = system_prompt + '''You are expected to compare and analyze the information between Data Safety and Privacy Policy to clarify the issue that 
        ...
        Privacy Policy information provided. Note: always gives me the result (0 or 1, 1 is yes, 0 is no) in the form below: {"inconsistent": (0 or 1)}. 
...
def my_loop_csv(input_csv_path, output_csv_path):
    with open(input_csv_path, "r", newline="", encoding="utf-8") as csvfile, open(
        ...
        for index, row in enumerate(reader):
            ...
            gpt_result = ask_remote_api(sysprompt, row[4], row[5])
            row[headers.index("result")] = remove_empty_lines(
                gpt_result
            )
            writer.writerow(row)
            print("~~~~~~~~~~~~~~ Success ~~~~~~~~~~~~~~\n")
...
my_loop_csv(input_csv_path, output_csv_path)
